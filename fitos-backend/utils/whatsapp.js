...
async function sendWhatsApp(phone, template, vars = {}, gymId = null) {
  ...
  try {
    ...
    await WhatsappLog.create({ gym_id: gymId, recipient_phone: to, template_name: template, status: 'sent', meta_message_id: messageId }).catch(() => {});
    return { ok: true, messageId };
  } catch (e) {
    const detail = e.response?.data?.error?.message || e.message;
    console.error(`[WA] Send failed (${template} -> ${to}):`, detail);
    await WhatsappLog.create({ gym_id: gymId, recipient_phone: to, template_name: template, status: 'failed', error_detail: detail }).catch(() => {});
